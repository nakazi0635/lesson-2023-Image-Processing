...
int main (int argc, const char* argv[]) {
    ...
    cv::Mat dst_img;
    cv::cvtColor(sourceImage, dst_img, cv::COLOR_BGR2HSV);
    cv::Vec3i s1; //int型の3要素配列s1
    for (int y=0; y<sourceImage.rows; y++) {
        for (int x=0; x<sourceImage.cols; x++) {
            s = dst_img.at<cv::Vec3b>(y, x); //"sourceImage"の画素(x,y)の画素値を読み込んで"s"に格納 s[0]=0; //"s[0]"に0を代入
            if((s[0]<70 || s[0]>90 || s[1]>255 || s[2]>200)){
                s1[0]=0;
                s1[1]=0;
                s1[2]=0;
                s=s1; //int型をunsignedchar型に丸める
                dst_img.at<cv::Vec3b>(y, x) = s; //"resultImage"の画素(x,y)に画素値"s"を書き込み
            }
        }
    }
    cv::cvtColor(dst_img, resultImage, cv::COLOR_HSV2BGR);
