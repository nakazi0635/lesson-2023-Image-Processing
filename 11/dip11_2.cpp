...
void change_face_color(cv::Mat& faceImage, cv::Mat& hsvImage, cv::Rect rect)
{
    ...
    for(int j=rect.y -50; j<rect.y+rect.height+20; j++) {
		if(j<0 || j>= hsvImage.rows) continue;
        for(int i=rect.x-50; i<rect.x+rect.width+20; i++) {
...
int main(int argc, char* argv[])
{
    ...
    while(1){
        ...
        for (int i = 0; i < faces.size(); i++) {
            ...
            //     cv::Point(face.x-50, face.y-50),
            //     // cv::Point(face.x + face.width, face.y + face.height),
            //     cv::Point(face.x + face.width+20, face.y + face.height+20),
            //     CV_RGB(255, 0, 0),
            //     3, cv::LINE_AA);
            }
