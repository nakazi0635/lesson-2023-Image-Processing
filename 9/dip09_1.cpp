...
int main (int argc, char* argv[])
{
    ...
    while (1) {
        ...
        cv::HoughCircles(grayImage, circles, cv::HOUGH_GRADIENT, 1, 40, 30, 11, 10, 20);
        ...
        for (int i=0; i<MIN(circles.size(), 200); i++) {
            ...
            cv::circle(frameImage, cv::Point(x0,y0), r, cv::Scalar(255,0,0), 2, 8, 0); //円の描画 
        }

        cv::putText(
            frameImage,
            "Count:  "+ std::to_string(circles.size()),
            cv::Point(50,200),
