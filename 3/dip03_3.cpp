...
int main(int argc, const char *argv[])
{
    ...
    while (1)
    {
        ...
        cv::resize(captureImage, frameImage, frameImage.size());
        cv::resize(captureImage2, frameImage2, frameImage.size());
        // 6画像処理
        // cv::cvtColor(frameImage, grayImage, cv::COLOR_BGR2GRAY);
        ...
        for (int y = 0; y < frameImage.rows; y++)
        {
            for (int x = 0; x < frameImage.cols; x++)
            {
                ...
                {
                    s0 = hsvImage2.at<cv::Vec3b>(y, x); // int型をunsignedchar型に丸める
                    hsvImage.at<cv::Vec3b>(y, x) = s0;  //"resultImage"の画素(x,y)に画素値"s"を書き込み
                }
            }
        }
        // cv::imshow("Frame", frameImage);
