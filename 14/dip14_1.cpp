//g++ dip14_1.cpp -std=c++11 -framework OpenGL -framework GLUT `pkg-config --cflags --libs opencv4` -Wno-deprecated
...
void display()
{
    ...
    for (std::size_t i = 1; i < pointVec.size(); i++) {
        ...
        // 点の情報を出力します。
        std::cout << "配列の中身 x: " << currentPoint.x << ", y: " << currentPoint.y << std::endl;
