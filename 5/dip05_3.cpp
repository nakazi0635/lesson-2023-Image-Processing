...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        cv::filter2D(medianImage, grayImage, grayImage.depth(), kernel);
        ...
        for (int j = 0; j < grayImage.rows; j++)
        {
            ...
            {
                unsigned char filteredValue = grayImage.at<unsigned char>(j, i);
                if (filteredValue > 200){
                    cv::line(originalImage2, cv::Point(j,i), cv::Point(j-50,i-50), cv::Scalar(0,0,0), 1, 8);
                }
            }
        }
